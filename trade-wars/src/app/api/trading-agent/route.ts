...
import { analyzeTradingOpportunity } from '@/lib/openai/tradingAgent';
...
export async function POST(request: Request) {
  try {
    console.log('🤖 Trading agent analysis started...');

    // Get OpenAI agent configuration
    const agentConfig = getAgentConfig('openai');
    if (!agentConfig) {
      throw new Error('OpenAI agent configuration not found');
    }
    ...
    console.log('🧠 Analyzing market with AI...');
    const result = await analyzeTradingOpportunity(marketData);
    console.log(`✓ AI Decision received with ${result.decision.actions.length} actions.`);
