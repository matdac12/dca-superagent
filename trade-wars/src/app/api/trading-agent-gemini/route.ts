...
import { analyzeTradingOpportunityGemini } from '@/lib/openrouter/tradingAgentGemini';
...
export async function POST(request: Request) {
  try {
    console.log('🤖 Trading agent analysis started (Gemini 2.5 Flash)...');

    // Get Gemini agent configuration
    const agentConfig = getAgentConfig('gemini');
    if (!agentConfig) {
      throw new Error('Gemini agent configuration not found');
    }
    ...
    console.log('🧠 Analyzing market with Gemini AI...');
    const result = await analyzeTradingOpportunityGemini(marketData);
    console.log(`✓ Gemini Decision received with ${result.decision.actions.length} actions.`);
    ...
    console.error('❌ Trading agent error (Gemini):', error);
